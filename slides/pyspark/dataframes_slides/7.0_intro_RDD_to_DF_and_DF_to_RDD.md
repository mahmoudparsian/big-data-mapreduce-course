...
>>> #-------------------------------------------------
>>> triplets = [('alex', 30, 30000), ('jane', 40, 40000), 
...
>>> key_value = [('alex', 30, 30000), ('jane', 40, 40000), ('mat', 60, 60000), ('janet', 60, 90000), ('Mo', None, None)]
>>>
>>> rdd = sc.parallelize(key_value)
>>>
>>> rdd.collect()
[('alex', 30, 30000), ('jane', 40, 40000), ('mat', 60, 60000), ('janet', 60, 90000), ('Mo', None, None)]
>>> rdd2 = rdd.map(lambda x: Row(name=x[0], age=x[1], salary=x[2]))
>>>
>>> rdd2.collect()
[Row(name='alex', age=30, salary=30000), Row(name='jane', age=40, salary=40000), Row(name='mat', age=60, salary=60000), Row(name='janet', age=60, salary=90000), Row(name='Mo', age=None, salary=None)]
>>> df = rdd2.toDF()
>>> df
...
## Example: create a DataFrame from a Python Collection
```
>>> countries = [
('USA', 20), 
('USA', 80), 
...
>>> column_names = ['country', 'score']
>>> column_names
['country', 'score']
>>>
>>> df = spark.createDataFrame(countries, column_names)
>>>
>>> df.show()
+-------+-----+
|country|score|
...
>>> df.createOrReplaceTempView("countries_table")
>>>
>>> spark.sql("select * from countries_table").show()
+-------+-----+
|country|score|
+-------+-----+
|    USA|   20|
...
>>> avg_df = spark.sql("select country, avg(score) as avg_score from countries_table GROUP BY country")
>>> avg_df.show()
+-------+---------+
|country|avg_score|
+-------+---------+
...
>>> avg_rdd.collect()
[
Row(country='USA', avg_score=50.0), 
Row(country='CHINA', avg_score=40.0), 
Row(country='CANADA', avg_score=70.0)
]
>>> spark.sql("select country, avg(score) as avg_score, min(score) as min_score, max(score) as max_score from countries_\
table GROUP BY country").show()
+-------+---------+---------+---------+
|country|avg_score|min_score|max_score|
+-------+---------+---------+---------+
|    USA|     50.0|       20|       80|
|  CHINA|     40.0|       30|       50|
| CANADA|     70.0|       70|       70|
